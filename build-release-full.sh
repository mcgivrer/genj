...
echo "🔨 Building genj in release mode with ZIP package..."
...
if [ -f "src/bin/build-package.rs" ] || [ -f "src/bin/build-package/main.rs" ]; then
    echo "📦 Creating ZIP package (build-package binary)..."
    cargo run --release --bin build-package
    echo "✨ ZIP package generated by build-package binary"
fi
...
if [ "$OS_NAME" = "Linux" ]; then
    if [ -x ./build_deb.sh ]; then
        echo "📦 Creating .deb package (Linux)..."
        ...
        echo "⚠️  build_deb.sh not found or not executable — skipping .deb creation"
    fi
else
    echo "ℹ️  Detected system: $OS_NAME — .deb creation only on Linux."
fi
